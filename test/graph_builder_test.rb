...
class Thing
  ...
  def self.relations(things)
    things.inject([]) {|res,t| r = t.relations; res +=r unless r.empty?; res }
  end

  def self.relations_s(things)
    relations(things).map{|r|"#{r.first}=>#{r.last}"}.join(",")
  end
...
require File.dirname(__FILE__) + '/../lib/graph_builder'

class GraphBuilderTest < Test::Unit::TestCase
  ...
  def test_simple
    ...
    GraphBuilder::Builder.build do |b|
      b.add t1
      assert_not_nil t2 = b.add(Thing2.new)
      assert_instance_of Thing2, t2
      assert_not_nil tt = b.add(TestThing.new)
      assert_instance_of TestThing, tt
    end

    r = Thing.relations([t1,t2,tt])
  ...
  def test_branch
    ...
    r = Thing.relations([t1,t2,t11,t12])
  ...
  def test_branch_chain
    ...
    r = Thing.relations([t1,t2,t11,t12,t21,t22])
    assert r.include? [t1,t11]
  ...
  def test_branch_chain2
    ...
    r = Thing.relations([t1,t2,t11,t12,t21,t23,t31,t32])
    assert r.include? [t1,t11]
    assert r.include? [t1,t21]
  ...
  def test_children
    ...
    r = Thing.relations(t.values.flatten)
